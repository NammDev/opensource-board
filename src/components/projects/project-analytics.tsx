...
export default function ProjectAnalytics({ project }: { project: EnrichedProjectProps }) {
  return (
...
async function ProjectAnalyticsRSC({ project }: { project: EnrichedProjectProps }) {
  const { links } = project

  // if project created less than 3 days ago, it's a newly added project
  const newlyAddedProject =
    new Date(project.createdAt).getTime() > Date.now() - 3 * 24 * 60 * 60 * 1000
  ...
  const chartData = analytics[0].map((data: { start: string; clicks: number }, i) => {
    return {
  ...
  return (
    <ProjectAnalyticsClient
      chartData={chartData}
      categories={[links[0].type, links[1] ? links[1].type : '']}
