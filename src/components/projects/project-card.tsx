...
export default function ProjectCard(project: Project) {
  return (
    ...
    >
      <div className={cn('aspect-[5/2] w-full rounded-t-xl bg-gradient-to-tr', project.gradient)} />
      ...
      <div className='p-4'>
        <div className='flex items-center space-x-1'>
          ...
          {project.verified && <BadgeCheck className='h-6 w-6 text-white' fill='#1c9bef' />}
        </div>
        <p className='mt-2 line-clamp-3 text-sm text-gray-500'>{project.description}</p>
      </div>
    </Link>
  )
}
