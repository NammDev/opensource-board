...
export function EditGradientPopoverClient({ project }: { project: EnrichedProjectProps }) {
...
const EditGradientForm = ({ project }: { project: EnrichedProjectProps }) => {
  ...
  const [state, formAction] = useFormState<FormResponse, FormData>(editGradient, null)
...
const Indicator = () => {
  ...
  return pending ? <LoadingSpinner className='h-4 w-4' /> : <Tick className='h-4 w-4 text-black' />
}
