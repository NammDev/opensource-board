...
import { FormResponse, editTeamSchema, selectUserSchema } from '@/lib/actions/utils'
...
export default function EditTeamForm({
  ...
  const [users, setUsers] = useState<EnrichedProjectProps['users']>(projectUsers)
  ...
  const [state, formAction] = useFormState<FormResponse & { user?: User }, FormData>(selectUser, {
    status: 'success',
    message: '',
  })
      ...
      setUsers((prev) => {
        ...
        return [
          ...prev,
          {
            ...
            image: foundUser.image ?? '', // Add this line to ensure image is always a string
          },
        ]
      })
    }
  ...
  return (
    <div className='flex flex-col space-y-4 bg-gray-50 px-4 py-8 md:px-16'>
      ...
      <div className='flex flex-col divide-y divide-gray-200 rounded-lg border border-gray-200 bg-white px-4 py-2'>
        {users.map((user) => (
          <div key={user.id} className='group relative flex items-center space-x-3 p-2'>
            <Image
              width={40}
              height={40}
              src={user.image}
...
const EditTeamFormPseudo = ({
  ...
  const [state, formAction] = useFormState<FormResponse, FormData>(editTeamWithUsers, null)
  ...
  }, [project.slug, router, setShowEditTeamModal, state])

  return (
    <form action={formAction}>
      <input {...register('projectId')} type='hidden' value={project.id} readOnly />
      <FormButton />
    </form>
  )
}
