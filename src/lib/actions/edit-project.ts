...
export async function editProject(_prevState: any, data: FormData): Promise<FormResponse> {
  try {
    const { name, description, github, website, projectId } = editProjectSchema.parse({
      name: data.get('name'),
    ...
    if (props.name !== name || props.description !== description) {
      await Promise.all([
        ...
        typesense().collections('projects').documents(projectId).update({ name, description }),
      ])
    }
